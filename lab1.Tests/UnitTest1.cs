...
namespace lab1.Tests
{
    public class UnitTest1
    {
        ...
        {
            string[] lines = { "1345 2584", "2584 683" }; // Менше 6 рядків

            var ex = Record.Exception(() => Program.ValidateInput(lines));
            Assert.NotNull(ex);
            Assert.IsType<InvalidOperationException>(ex);
            Assert.Equal("The number of input lines must be exactly 6.", ex.Message);
        }
        ...
        public void Test_InvalidNumberFormat()
        {
            ...
            Assert.NotNull(ex);
            Assert.IsType<InvalidOperationException>(ex);
            Assert.Equal("Both values on each line must be positive integers.", ex.Message);
        }
